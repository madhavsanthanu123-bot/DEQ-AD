...
def main():
    ...
    # DataLoaders
    train_dataset = OASISDataset(results_dir / "train_manifest.csv")
    val_dataset = OASISDataset(results_dir / "val_manifest.csv")
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_cls_loss /= n_val
        val_cdr_loss /= n_val
        v_acc, v_prec, v_rec, v_f1, v_auc = safe_metrics(val_labels, val_preds, val_probs)
        ...
        checkpoint_data = {
            ...
            "validation_metrics": {"f1": v_f1, "loss": val_loss},
            "random_seed": 42,
            "configuration": {"batch_size": 2, "lr": 1e-4}
        }
        
        if v_f1 > best_val_f1:
            best_val_f1 = v_f1
            torch.save(checkpoint_data, chkpt_dir / "best_model.pt")
    ...
    # Save last model
    torch.save(checkpoint_data, chkpt_dir / "last_model.pt")
